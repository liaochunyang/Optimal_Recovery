...
% Here we produce Figure 4(b) in the main text.
%% Load a small graph
clear all;clc;
load smallgraphs/polbooksA.mat
rng(1)            % comment out this line to produce a different plot
...
range_nl = 5:5:n/2;
Monte = 50;
err_loc = zeros(Monte,length(range_nl));
err_loc_over = zeros(Monte,length(range_nl));
...
for i=1:length(range_nl)
    ...
    % Quantity of interest $Q(f)=f|_{V_u}
    Q = zeros(n-nl,n);
    ...
    for j=1:Monte
        ...
        % Global OR: using an estimated eta
        ...
        % Local OR: using true eta
        g = @(t) ((1-t)*full(L) + t*Lambda'*Lambda)\(t*Lambda'*lab);
        g1 = @(t) g(t)'*L*g(t)-epsilon^2/eta^2*norm(Lambda*g(t)-lab)^2;
        loc_tau = fzero(g1,0.6);
        x_hat = Q*(((1-loc_tau)*full(L) + loc_tau*Lambda'*Lambda)\(loc_tau*Lambda'*lab));
        err_loc(j,i) = norm(x(setdiff(1:n,l))-x_hat);
        
        % Local OR: using an overestimated eta
        g = @(t) ((1-t)*full(L) + t*Lambda'*Lambda)\(t*Lambda'*lab);
        g1 = @(t) g(t)'*L*g(t)-epsilon^2/eta^2*norm(Lambda*g(t)-lab)^2;
        loc_tau_over = fzero(g1,0.6);
        x_hat = Q*(((1-loc_tau_over)*full(L) + loc_tau_over*Lambda'*Lambda)\(loc_tau_over*Lambda'*lab));
        err_loc_over(j,i) = norm(x(setdiff(1:n,l))-x_hat);
        ...
        err_lb(1) = norm(x(setdiff(1:n,l)) - mean(lab)*ones(n-nl,1));
        for k=2:T+1
            tau = range_tau(k);
            % Regularization map
...
plot(range_nl,mean(err_loc),'g-x',range_nl,mean(err_loc_over),'k-^','Linewidth',1)
plot(range_nl,mean(err_best),'b-*','Linewidth',1)
xticks(5:5:n/2)
xlim([5,n/2])
legend('Naive baseline (\tau=0)','Global OR (true \eta)','Global OR (overestimated \eta)','Local OR (true \eta)','Local OR (overestimated \eta)','Best regularization','Fontsize',20)
...
%set(gca, 'Position', [-0.1,0.2,1.2,0.85])
